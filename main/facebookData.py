...
from models import Advertisers, Datas, Adwords, CampaignRelations, Rebate, Offer, User, Customers, PlatformOffer
import json
import datetime
import re
...
@facebookDate.route('/api/dashboard', methods=['POST','GET'])
def dashboard():
...
#dashboard中表格数据
@facebookDate.route('/api/dashboard/table', methods=["POST","GET"])
def dbTable():
    if request.method == "POST":
        data = request.get_json(force=True)
        start_date = data['start_date']
        ...
        facebook_apple_data = []
        adwords_data = []
        fb_ap_datas = Datas.query.filter(Datas.date >= start_date, Datas.date <= end_date).with_entities(Datas.offer_id,Datas.type,Datas.date,sa.func.sum(Datas.revenue),sa.func.sum(Datas.cost),sa.func.sum(Datas.profit),sa.func.sum(Datas.impressions),sa.func.sum(Datas.clicks),sa.func.sum(Datas.conversions))
        fb_result = fb_ap_datas.group_by(Datas.date,Datas.offer_id,Datas.type).all()
        for i in fb_result:
            offer = Offer.query.filter_by(id=i[0]).first()
            users = User.query.filter_by(id=offer.user_id).first()
            sale = Customers.query.filter_by(id=offer.customer_id).first()
            platform = PlatformOffer.query.filter_by(offer_id=i[0],platform=i[1]).first()
            ...
            facebook_apple_data += [
                {
                    ...
                    "clicks": int(i[7]),
                    "conversions": int(i[8]),
                    "BD": sale.company_name,
                    "advertiser": users.name,
                    "GEO": country.split(','),
                    "system": offer.os
                }
            ]
        ad_datas = Adwords.query.filter(Adwords.date >= start_date, Adwords.date <= end_date).with_entities(Adwords.offer_id,Adwords.date,sa.func.sum(Adwords.revenue),sa.func.sum(Adwords.cost),sa.func.sum(Adwords.profit),sa.func.sum(Adwords.impressions),sa.func.sum(Adwords.clicks),sa.func.sum(Adwords.conversions))
        as_result = ad_datas.group_by(Adwords.date,Adwords.offer_id).all()
        for i in as_result:
            adwords_data += [
                {
                    "offerId": i[0],
        ...
        return json.dumps({"code":200,"fecebook_apple":facebook_apple_data,"adwords":adwords_data})
